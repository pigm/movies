...
public abstract class BaseListAdapter<T, V extends BaseViewHolder> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    protected static final int VIEW_TYPE_HEADER = 0000;
        ...
        try {
                ...
                if (v.getChildAt(v.getChildCount() - 1) != null) {
                    if ((scrollY >= (v.getChildAt(v.getChildCount() - 1).getMeasuredHeight() - v.getMeasuredHeight())) &&
                        ...
                        if (attempToLoadMore(recyclerView, mOnLoadMoreListener)) {
                            v.postDelayed(() -> v.fullScroll(View.FOCUS_DOWN), 100);
                        }
                    }
                }
            });
            ...
            recyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
                ...
                public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                    ...
                    if (lastVisiblePosition != RecyclerView.NO_POSITION && getItemCount() > 0 && lastVisiblePosition >= getItemCount() - 3) {
                        if (attempToLoadMore(recyclerView, mOnLoadMoreListener)) {
    ...
    public void setList(T items, boolean notify) {
        lista = items;
        if (notify)
            notifyDataSetChanged();
    }
